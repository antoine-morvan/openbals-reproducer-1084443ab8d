...
DEST=$(uname -s)/$(uname -m)/openblas_${COMPILER_NAME}-${COMPILER_VERSION} ./openblas.sh ${COMPILER_NAME}-${COMPILER_VERSION} |& tee $(uname -m):${COMPILER_NAME}:${COMPILER_VERSION}:02-openblas.log